...
fn Multiplier_0_create(soffset: usize, coffset: usize, ctx: &mut Context, componentName: String, componentFather: usize){
...
Fr_sub!(expaux[2],ctx.signalValues[mySignalStart + 1],ctx.circuitConstants[1]); // line circom 32
Fr_div!(expaux[0],ctx.circuitConstants[1],expaux[2]); // line circom 32
Fr_copy!(aux_dest,expaux[0]);
}
Fr_sub!(expaux[2],ctx.signalValues[mySignalStart + 1],ctx.circuitConstants[1]); // line circom 33
...
Fr_sub!(expaux[2],ctx.signalValues[mySignalStart + 2],ctx.circuitConstants[1]); // line circom 35
Fr_div!(expaux[0],ctx.circuitConstants[1],expaux[2]); // line circom 35
Fr_copy!(aux_dest,expaux[0]);
}
Fr_sub!(expaux[2],ctx.signalValues[mySignalStart + 2],ctx.circuitConstants[1]); // line circom 36
...
Fr_mul!(expaux[0],ctx.signalValues[mySignalStart + 1],ctx.signalValues[mySignalStart + 2]); // line circom 38
Fr_copy!(aux_dest,expaux[0]);
}
}
